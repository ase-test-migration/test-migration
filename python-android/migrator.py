...
def assert_equal_attribute_mapper(source_attribute):
    dest_attribute = ""
...
class TestCase:
    ...
    def add_oracle_event(self, action, state, oracle_type, oracle_input):
        event = OracleEvent(action, state, oracle_type, oracle_input)
        self.events.append(event)
...
class Migrator:
    def __init__(self, source_test_folder_path):
        ...
        if source_test_folder_path.endswith("/"):
            raise Exception("Input Folder shouldn't contain / in the end. Remove it to continue.")
        ...
        self.scroll_down_event = DestEvent("scroll_down", None, None, None)
        self.test_generator = test_generator.TestGenerator(source_test_folder_path, self.source_test_folder_path.split("/")[-1])
    ...
    def make_source_sequence(self):
        ...
        #os.makedirs(os.path.join("outputs/ios-to-android", test_name))
        print("test name: " + test_name)
        for state in sorted(os.listdir(self.source_test_folder_path)):
            text_input = ""
            if not os.path.isdir(os.path.join("outputs/ios-to-android", test_name, state)):
                os.makedirs(os.path.join("outputs/ios-to-android", test_name, state))
            if state == ".DS_Store":
                continue
            # print("state: " + state)
            with open(os.path.join(self.source_test_folder_path, state,
                ...
                if metadata_file_event=="oracle":
                    ...
                    if oracle_type == "element_invisible":
                        ...
                        text_ids_dict = json.loads(widget_text_ids)
                        text_ids_dict = text_utils.preprocess_info_dict(text_ids_dict)
                        test_case.add_event("oracle-element_invisible", state, widget_image_path, text_ids_dict, text_input)
                ...
                elif metadata_file_event=="back":
                    test_case.add_event(metadata_file_event, state, "", {}, "")
                else:
                    widget_image_path = ((metadata_file_image_path.split(": "))[1])[:-2]
    ...
    def  find_best_action(self, current_state, event, state_name_path, i):
        source_widget_path = "python-ios/"+event.image_path
        destination_widgets_path = state_name_path+"/pngs"
        image_based_ranking, visual_scores, source_ocr = self.get_actions_visual_similarity_ranking(source_widget_path, current_state)
        if source_ocr !="":
        ...
        if event.action == "oracle-assert_equal":
            ...
            dest_attribute = assert_equal_attribute_mapper(attribute)
            new_text_input = dest_attribute+" : "+value
            next_event = DestEvent(event.action, next_event_widget.get_exec_id_type(), next_event_widget.get_exec_id_val(), new_text_input)
        else:
            print(next_event_widget.get_exec_id_val())
            next_event = DestEvent(event.action, next_event_widget.get_exec_id_type(), next_event_widget.get_exec_id_val(), event.text_input)

        # # image_based_ranking = keypoint_comparator.find_matching_widget(source_widget_path, destination_widgets_path)
    ...
    def get_element_max_similarity(self, element_text_dict, event_text_dict):
        ...
        for key1,event_text in event_text_dict.items():
            for key2,element_text in element_text_dict.items():
                if event_text!="" and element_text!="" and event_text is not None and element_text is not None:
    ...
    def get_actions_visual_similarity_ranking(self, source_widget_path, destination_state):
        ...
        for element in destination_state.nodes:
            if element.interactable:
                ...
                exact_text_match, common_words, proximity_score, size_similarity_score, image_similarity_score, edit_distance, ocr_output, source_ocr, img = similarity_calculator.calc_score(source_widget_path, dest_widget_path, 2)
        ...
        for ranked in sort_by_scores:
            print(ranked.exec_identifier)
        print(sort_by_scores)
        return sort_by_scores, element_score, source_ocr
...
if __name__ == "__main__":
    ...
    print("Android migration running time " + str(end - start) + " seconds")
