...
class TestGenerator:
    ...
    def add_step(self, event):
        ...
        if event.action == "oracle-text_exists":
            ...
            if event.exec_id_type == "xPath":
                ...
                new_event["tag"] = event.exec_id_val[0]

            if event.exec_id_type == "resource-id":
                new_event["type"] = "resource-id"
                new_event["tag"] = event.exec_id_val
    ...
    def save_test(self):
        ...
        filename = "generated_tests/{}/{}/ios-to-android/generated_test.json".format(self.app_name, self.test_name)
        print(output)
